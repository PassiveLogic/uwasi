...
Publish a GitHub release with a stable tag such as `1.5.0` or `v1.5.0`.
The Release workflow derives the npm version from the tag and publishes to `latest`;
no manifest version bump is needed. Prerelease tags are not supported.
To publish an existing, unpublished tag, run Release manually from `main` and
enter the tag in the `tag` input.
