...
// wasi-testsuite has no fd_sync case, so the sync ordering and the
...
// The unlink is the commit point, so the backend has to guarantee, in order:
...
//      itself becomes durable, leaving no crash window that shows a stale
//      journal;
//   3. a fresh worker re-initializing from the same store never resurrects
//      an unlinked journal, and still sees a journal that was not unlinked
//      (a hot journal) byte-for-byte.
